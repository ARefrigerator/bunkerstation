	...
	if(href_list["interaction"])
		...
		if (href_list["interaction"] == "bow")
			...
			if(((!istype(P.loc, /obj/structure/closet)) || (H.loc == P.loc)) && hashands)
				...
				H.attack_log += text("\[[time_stamp()]\] <font color='red'>petted [P.name] ([P.ckey])</font>")
				P.attack_log += text("\[[time_stamp()]\] <font color='orange'>was petted by [H.name] ([H.ckey])</font>")
			...
			if( ((Adjacent(P) && !istype(P.loc, /obj/structure/closet)) || (H.loc == P.loc)) && mouthfree && mouthfree_p  && (H.species.flags & HAS_LIPS) && (P.species.flags & HAS_LIPS))
				if (H.lust == 0)
					...
					H.visible_message("<B>[H]</B> gives a more intense kiss to <B>[P]</B>.")
					if (istype(P.loc, /obj/structure/closet))
						P.visible_message("<B>[H]</B> gives a more intense kiss to <B>[P]</B>.")
			else if (mouthfree)
			...
			if(((Adjacent(P) && !istype(P.loc, /obj/structure/closet)) || (H.loc == P.loc)) && hashands)
				H.visible_message("<span class='danger'>[H] knocks [P] upside the head!</span>")//Knocks?("<span class='danger'>[H] дает [P] подзатыльник!</span>")
				if (istype(P.loc, /obj/structure/closet))
			...
			if(((Adjacent(P) && !istype(P.loc, /obj/structure/closet)) || (H.loc == P.loc)) && mouthfree)
				...
				H.attack_log += text("\[[time_stamp()]\] <font color='red'>spitted [P.name] ([P.ckey])</font>")
				P.attack_log += text("\[[time_stamp()]\] <font color='orange'>was spitted by [H.name] ([H.ckey])</font>")
			...
			if (H.loc == P.loc && isnude_p && isnude && haspenis && hasanus_p)
				if (H.erpcooldown == 0)
					...
					var/message = pick("it's not erect...")//, "Как-то нет желани[ya]...", "Что-то не охота...", "Нет, не сейчас.")
					to_chat(H, message)
			...
			if (H.loc == P.loc && isnude_p && isnude && haspenis && hasanus_p)
				if (H.erpcooldown == 0)
					...
					var/message = pick("you can't do that yet...")//"Не хочетс[ya] мне...", "Как-то нет желани[ya]...", "Что-то не охота...", "Нет, не сейчас.")
					to_chat(H, message)
			...
			if (H.loc == P.loc && isnude && mouthfree_p && haspenis)
				if (H.erpcooldown == 0)
					...
					var/message = pick("can't do that yet...")//"Не хочетс[ya] мне...", "Как-то нет желани[ya]...", "Что-то не охота...", "Нет, не сейчас.")
					to_chat(H, message)

		else if (href_list["interaction"] == "mount")
			if (H.loc == P.loc && isnude && isnude_p && haspenis_p && hasvagina)
				if (P.lust <= 0)
					var/message = pick("you have no lust now...")//"Инструмент не переведен в рабочее состо[ya]ние...", "У него еще не встал...", "А он лежит...", "Никак не насадитьс[ya]...")
					to_chat(H, message)
...
/mob/living/carbon/human/proc/get_age_pitch()//Interactions
