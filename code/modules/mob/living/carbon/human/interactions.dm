...
/mob/living/carbon/human/make_interaction()
	...
	if (hashands)
		...
		if (Adjacent(P))
			...
			dat +=  {"• <A href='?src=\ref[usr];interaction=knock'><font color=red>Knock?</font></A><BR>"}
	...
	if (H.gender == MALE)
		...
		if (H.species.genitals)
			if (hole == "mouth" || H.zone_sel.selecting == "mouth")
				message = pick("cums right in [P]'s mouth.")//, "целитс[ya] в лицо [P], стрел[ya]ет тугой струёй малафьи, но промахиваетс[ya].", "разбрызгивает сем[ya] на лицо [P].", "кончает на пол.")

			else if (hole == "vagina")
				message = pick("cums in [P]'s pussy")//кончает в [P]", "резко выт[ya]гивает член из [P], а затем спускает на пол.", "проникает в [P] последний раз, затем содрагаетс[ya]. Сперма медленно вытекает из щели [P].")

			else if (hole == "anus")
				message = pick("cums in [P]'s asshole.")//, "выдергивает член из [P], а затем обильно кончает на [P.gender == MALE ? "его" : "её"] попку.", "выт[ya]гивает член из задницы [P] и сразу же спускает на пол.")

			else if (hole == "floor")
				message = "cums on the floor!"

		else
			message = pick("cums!", "orgams!")//"извиваетс[ya] в приступе оргазма", "прикрывает глаза и мелко дрожит", "содрагаетс[ya], а затем резко расслабл[ya]етс[ya]", "замирает, закатив глаза")
		...
		message = pick("aaaaaa")//"извиваетс[ya] в приступе оргазма", "прикрывает глаза и мелко дрожит", "содрагаетс[ya], а затем резко расслабл[ya]етс[ya]", "замирает, закатив глаза")
		H.visible_message("<B>[H] [message].</B>")
		if (istype(P.loc, /obj/structure/closet))
			P.visible_message("<B>[H] [message].</B>")
...
mob/living/carbon/human/proc/fuck(mob/living/carbon/human/H as mob, mob/living/carbon/human/P as mob, var/hole)
	...
	switch(hole)
		...
		if("blowjob")
			...
			if (prob(P.potenzia))
				H.oxyloss += 3
				H.visible_message("<B>[H]</B> goes in deep on <B>[P]</B>.") //[pick("давитс инструментом <B>[P]</B>", "задыхаетс[ya]", "корчитс[ya] в рвотном позыве")].")
				if (istype(P.loc, /obj/structure/closet))
					P.visible_message("<B>[H]</B> goes in deep on <B>[P]</B>.")//"<B>[H]</B> [pick("давитс инструментом <B>[P]</B>", "задыхаетс[ya]", "корчитс[ya] в рвотном позыве")].")
		...
		if("mount")
			...
			if (H.lastfucked != P || H.lfhole != hole)
				message = pick("begins to hop on [P]'s dick")//"осторожно насаживаетс[ya] на половой орган [P]")
				H.lastfucked = P
				H.lfhole = hole
				add_logs(P, H, "fucked")
...
/obj/item/weapon/dildo/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	if(istype(M, /mob/living/carbon/human) && user.zone_sel.selecting == "groin" && M.is_nude())
		if (hole == "vagina" && hasvagina)
			...
			if (user == M)
				message = pick("fucks their ass")//, "заталкивает [rus_name] себе в анус", "чистит свой дымоход, использу[ya] [rus_name]")
			else
				message = pick("fucks [M]'s asshole")//, "заталкивает [rus_name] [M] в анус", "чистит дымоход [M], использу[ya] [rus_name]")
